...
const deleteCategory = async (req, res) => {
  try {
    ...
    if (category.provider.toString() !== providerId.toString()) return res.status(403).json({ success: false, message: "مش مسموح" });
    const servicesUnder = await Service.countDocuments({ parent: req.params.id, provider: providerId });
    if (servicesUnder > 0) return res.status(409).json({ success: false, message: `يوجد ${servicesUnder} خدمة تحت هذا التصنيف` });
...
const getServices = async (req, res) => {
  try {
    ...
    if (req.query.category) result = result.filter((ps) => ps.service?.parent?._id?.toString() === req.query.category);
    const totalServices = result.length;
    const fastServiceCount = result.filter((ps) => ps.fast_service).length;
    res.status(200).json({ success: true, count: totalServices, stats: { totalServices, fastServicePct: totalServices > 0 ? Math.round((fastServiceCount / totalServices) * 100) : 0, avgPrice: totalServices > 0 ? Math.round((result.reduce((s, ps) => s + ps.price, 0) / totalServices) * 100) / 100 : 0 }, data: result });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
};

const getServiceById = async (req, res) => {
  try {
    const ps = await ProviderService.findOne({ _id: req.params.id, provider: req.shop._id }).populate({ path: "service", select: "name description icon parent duration_hours unit", populate: { path: "parent", select: "name icon" } });
...
const updateService = async (req, res) => {
  try {
    ...
    await ps.save();
    const svcUpdates = {};
    ["name", "description", "icon", "duration_hours", "unit"].forEach((f) => { if (req.body[f] !== undefined) svcUpdates[f] = req.body[f]; });
    if (Object.keys(svcUpdates).length > 0) await Service.findOneAndUpdate({ _id: ps.service, provider: providerId }, svcUpdates);
    const populated = await ps.populate({ path: "service", select: "name description icon parent duration_hours unit", populate: { path: "parent", select: "name icon" } });
    res.status(200).json({ success: true, message: "تم تحديث الخدمة", data: populated });
  } catch (error) { res.status(500).json({ success: false, message: error.message }); }
};
